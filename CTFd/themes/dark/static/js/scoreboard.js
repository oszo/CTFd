...
function renderposition(account_id, cat) {
  for (var i = 0; i < position[cat].length; i++) {
    if (
      position[cat][i].account_id === account_id &&
      position[cat][i].state === "up"
    ) {
      return '<i class="fas fa-angle-up text-success"></i>';
    } else if (
      position[cat][i].account_id === account_id &&
      position[cat][i].state === "down"
    ) {
...
function updatescoresbycat(cat, catcount) {
  ...
  $.get(
    script_root +
      (cat === "all"
        ? "/api/v1/scoreboard"
        : "/api/v1/scoreboard/StatByCat/" + cat),
    function(response) {
      ...
      if (JSON.stringify(scorebard_data[cat]) !== JSON.stringify(teams)) {
        ...
        var element_id_prefix =
          "tab-" + replaceSpacialChar(cat) + "-tbody-account";
        for (var i = 0; i < teams.length; i++) {
          ...
          $("#" + element_id_prefix + team_id + "-solve div div").attr(
            "style",
            "width: " +
              team_solve +
          ...
          var current_solve = $(
            "#" + element_id_prefix + team_id + "-solve div div"
          ).text();
          ...
          if (current_solve !== team_solve) {
            ...
            setTimeout(function() {
              $("#" + element_id_prefix + team_id + "-solve div div").attr(
                "class",
                "progress-bar bg-promary"
              );
            }, 3000);
          }
          $("#" + element_id_prefix + team_id + "-solve div div").text(
            team_solve + "%"
          );
          ...
          setTimeout(function() {
            ...
            for (var i = 0; i < teams.length; i++) {
              var row =
                ...
                '<td id="{0}{1}-team"><a href="{2}/{3}/{4}">{5}</a></td>'.format(
                  element_id_prefix,
                  teams[i].account_id,
                  script_root,
                  user_mode,
                  teams[i].account_id,
                  htmlentities(truncatedstring(teams[i].name, 35))
                ) +
                '<td id="{0}{1}-solve"><div class="progress" style="height: 20px;"><div id="score-progress-bar" class="progress-bar bg-primary" role="progressbar" style="width: {2}%; -webkit-transition: width 2s; transition: width 2s;" aria-valuenow="{2}" aria-valuemin="0" aria-valuemax="100">{2}%</div></div></td>'.format(
                  element_id_prefix,
                  teams[i].account_id,
                  renderpercentwodigit(teams[i].solve, catcount)
                ) +
                '<td id="{0}{1}-score" class="text-right">{2}</td>'.format(
                  element_id_prefix,
                  teams[i].account_id,
                  teams[i].score
                ) +
                '<td id="{0}{1}-state" class="text-center">{2}</td>'.format(
                  element_id_prefix,
                  teams[i].account_id,
                  renderposition(teams[i].account_id, cat)
                ) +
                "</tr>";
              table.append(row);
            }
